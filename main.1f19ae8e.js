...
window.onload = function () {
  var navSpans = _toConsumableArray(document.querySelectorAll("nav > span"));

  var expandSpans = _toConsumableArray(document.querySelectorAll(".expand-parent"));

  var activeSpans = Object.keys(navSpans).reduce(function (acc, key) {
    return Object.assign(acc, _defineProperty({}, key, new Set([])));
  }, {});
  navSpans.forEach(function (navSpan, ix) {
    navSpan.addEventListener("mouseenter", function () {
      expandSpans[ix].classList.add("active");
      var uid = Date.now().toString().slice(5);
